...
import { Order, OrderWorkflowStatus } from '../models/Order.js';
...
const ORDER_STAGES: Array<{ status: OrderWorkflowStatus; title: string; description: string }> = [
...
export const updateOrderStatus = async (req: AuthRequest, res: Response): Promise<void> => {
  try {
    ...
    if (stepIndex !== -1) {
      ...
      if (note) order.timeline[stepIndex].description = note;
