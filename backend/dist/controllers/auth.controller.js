...
const generateTokens = (user) => {
    ...
    const token = jsonwebtoken_1.default.sign({ id: user._id, email: user.email, role: user.role, name: user.name }, secret, { expiresIn: '7d' });
...
const register = async (req, res) => {
    try {
        ...
        await Cart_js_1.Cart.create({ userId: user._id, items: [] });
        await Wishlist_js_1.Wishlist.create({ userId: user._id, studios: [], products: [] });
...
const login = async (req, res) => {
    try {
        ...
        const user = await User_js_1.User.findOne({ email: email.toLowerCase() }).select('+password');
        if (!user) {
        ...
        if (user.role === 'shop_owner') {
            const studio = await Studio_js_1.Studio.findOne({ ownerId: user._id });
            if (studio)
                studioId = studio._id;
        }
        const { token, refreshToken } = generateTokens(user);
...
const googleAuth = async (req, res) => {
    try {
        ...
        let user = await User_js_1.User.findOne({ email: email.toLowerCase() });
        if (!user) {
...
const getMe = async (req, res) => {
    try {
        ...
        let studio = null;
        if (user.role === 'shop_owner') {
            studio = await Studio_js_1.Studio.findOne({ ownerId: user._id });
        }
        res.json({
            success: true,
            user: {
